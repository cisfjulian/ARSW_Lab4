...
import edu.eci.arsw.blueprints.persistence.impl.SubS;
...
public class InMemoryPersistenceTest {
    ...
    public void getBlueprintByAuthor(){
        InMemoryBlueprintPersistence ibpp = new InMemoryBlueprintPersistence();
        Point[] pts0=new Point[]{new Point(12, 11),new Point(34, 42)};
        Blueprint bp=new Blueprint("jul", "stadium",pts0);
        Point[] pts1=new Point[]{new Point(15, 61),new Point(10, 12)};
        Blueprint bp1=new Blueprint("jul", "stadium2",pts1);
        try{
            ibpp.saveBlueprint(bp);
            ibpp.saveBlueprint(bp1);
            assertEquals(ibpp.getBlueprintsByAuthor("jul").size(), 2);
    ...
    public void subsampling(){
        Point[] pts0=new Point[]{new Point(13, 41),new Point(12, 40), new Point(15, 28), new Point(12, 41)};
        Blueprint bp=new Blueprint("jul", "stadium",pts0);
        SubS s = new SubS();
        Blueprint blp = s.filter(bp);
        assertEquals(blp.getPoints().get(0).getX(), 13);
        assertEquals(blp.getPoints().get(0).getY(), 41);
        assertEquals(blp.getPoints().get(1).getX(), 15);
        assertEquals(blp.getPoints().get(1).getY(), 28);
    }
